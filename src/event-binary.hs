...
import           Data.Monoid ((<>))
import           System.IO (openFile, hClose, IOMode(..), hSetBinaryMode)
...
instance Binary Params
instance Binary EventIn
instance Binary EventOut
...
{-
readFile' :: (Binary a) => FilePath
	 -> Producer' a (PS.SafeT IO) ()
readFile' file = PS.bracket
    (do h <- openFile file ReadMode
        hSetBinaryMode h True
	putStrLn $ C.concat ["{",C.pack file," open}"]
    ...
    (\h -> do
	  hClose h
	  putStrLn $ C.concat ["{",C.pack file," closed}"] )
    (\h -> L.hGet defaultChunkSize h)
    PB.fromHandle
-}

writeFile :: (Binary a) => FilePath
	   -> Consumer' a (PS.SafeT IO) ()
writeFile file = PS.bracket
    ...
    (\h -> do
	  hClose h
	  putStrLn $ C.concat ["{",C.pack file," closed}"])
    (\h -> for cat (liftIO . L.hPut h . encode))
...
main = do
    ...
    let saveIn = P.tee $ writeFile "saveintest.txt"
    let saveOut = P.tee $ writeFile "saveouttest.txt"
...
replayIO = do
    ...
    let loadIn =     takeLines (readFile "saveintest.txt")
        ...
        saveReplayOut = P.tee $ writeFile "savereplaytest.txt"
